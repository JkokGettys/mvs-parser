"""
Number of Stories Multiplier Parser for Marshall Valuation Service
Extracts story count multiplier tables from MVS PDF
...
def parse_stories_multiplier_table(pdf_path: str, start_page: int, end_page: int) -> Dict:
    """
    Parse number of stories multiplier tables from specified page range
    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for page_num in range(start_idx, end_idx):
                ...
                page_multipliers = parse_page_stories(page, page_text, display_page)
                
                if page_multipliers:
                    multipliers.extend(page_multipliers)
                    print(f"   [+] Found {len(page_multipliers)} story multiplier entries")
                else:
...
def parse_page_stories(page, page_text: str, page_num: int) -> List[Dict]:
    """
    Parse story multiplier entries from a single page
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            'metadata': {
                ...
                'description': 'Number of stories cost multipliers',
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python parse-stories-multiplier.py <pdf_path> <start_page> <end_page>")
    ...
    results = parse_stories_multiplier_table(pdf_path, start_page, end_page)
    
    if results['success']:
        output_dir = Path(__file__).parent.parent / "Tables" / "Multipliers" / "Stories"
        
        markdown_path = output_dir / "STORIES_MULTIPLIERS.md"
        save_to_markdown(results['multipliers'], str(markdown_path))
        
        json_path = output_dir / "stories_multipliers.json"
